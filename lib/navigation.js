...
module.exports = function () {
  return function (files, metalsmith, done) {
    ...
    for (const filepath in files) {
      ...
      const isUnique = !(navigation[section] || []).find(item => item.href === href)
      ...
      if (shouldAppearInMenu) {
        ...
        !(section in navigation) && (navigation[section] = [])
        navigation[section].push(item)
        navigation[section].sort(sortAlphabetically)
      }
    }
