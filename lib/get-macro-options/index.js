...
function getMacroOptionsJson (componentName) {
  try {
    let optionsFilePath = path.join("node_modules/hmrc-frontend/hmrc/components/", componentName, 'macro-options.json')
...
function getMacroOptions (componentName) {
  ...
  const nestedOptions = getNestedOptions(mappedOptions)
  const additionalComponents = getAdditionalComponentOptions(mappedOptions)
